...
class TumblrPost:
    ...
    def __init__(self, post_iid, post_id, post_url, post_date, post_title, post_body):
        self.iid = post_iid
        self.id = post_id
    ...
    if 'tags' in json_page:
        ...
        if found:            
            ...
            if lang == 'pl':
                ...
                post_iid = int(hashlib.md5(json_page['url'].encode('utf-8')).hexdigest(), 16)
                ...
                p = TumblrPost(post_iid, post_id, json_page['url'], post_date, title, body)
                all_posts.append(p)
...
def get_posts_from_blogs(blog_name, count_blogs):
    ...
    while start < total_count and total_count < 10000:
        ...
        if 'posts' in json_page:
            for post in json_page['posts']:
                if ln_ok:
                    ln_ok = get_and_save_post(post, all_posts)
...
def main():
    ...
    while len(links_to_blogs) != 0 and count < 100:
        get_posts_from_blogs(links_to_blogs.pop(), count)
